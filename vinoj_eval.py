...
if __name__ == "__main__":
    ...
    #data/squad1.1/dev-v1.1.json  data/qanta.dev.2018.json
    parser.add_argument("--batch_size", default=32, type=int)
    parser.add_argument("--topk", default=5, type=int)
    parser.add_argument("--learning_rate", default=1e-5, type=float)
    parser.add_argument("--split_rule", default="full", type=str)
    parser.add_argument("--scaling_param", default=1.1, type=float)
    ...
    # question_model = torch.load("models/guesser_question_encoder_last-c_32_1e-05_1.0_0.pth.tar").to(device)
    # context_model = torch.load("models/guesser_context_encoder_last-c_32_1e-05_1.0_0.pth.tar").to(device)
    
    # question_model = torch.load("models/guesser_question_encoder_last-c_32_1e-05_1.0_0.pth.tar").to(device)
    # context_model = torch.load("models/guesser_context_encoder_last-c_32_1e-05_1.0_0.pth.tar").to(device)
    
    question_model = torch.load("models/guesser_question_encoder_last-a_16_1e-05_1.0_0.pth.tar").to(device)
    context_model = torch.load("models/guesser_context_encoder_last-a_16_1e-05_1.0_0.pth.tar").to(device)
    ...
    ## get train dataset
    guesstrain = QantaDatabase(flags.train_data, dataset_name='squad')
    guessdev = QantaDatabase(flags.dev_data, dataset_name='squad')
    ...
    # pdb.set_trace()
    ## accuracy
    print(f'Top-{flags.topk} Retrieval accuracy Last: {num_correct_preds*100 / len(dev_dataset)}')
    ...
    with torch.no_grad():
        for i, batch in enumerate(dev_dataloader):
            ...
            last_sentences = torch.zeros((batch_questions.shape[0], batch_questions.shape[-1]), dtype=torch.long).to(device)
            for item_idx, valid_index_list in enumerate(valid_indices):
                last_idx = int(valid_index_list.split(',')[0])
                last_sentences[item_idx] = batch_questions[item_idx][last_idx]
            last_sentence_embeddings = question_model(last_sentences).pooler_output
    ...
    # pdb.set_trace()
    ## accuracy
    print(f'Top-{flags.topk} Retrieval accuracy First: {num_correct_preds *100 / len(dev_dataset)}')
...
#             last_sentences = torch.zeros((batch_questions.shape[0], batch_questions.shape[-1]), dtype=torch.long).to(device)
#             for item_idx, valid_index_list in enumerate(valid_indices):
#                 last_idx = valid_index_list.split(',')
#                 last_idx = int(last_idx[int(len(last_idx)/2)])
#                 last_sentences[item_idx] = batch_questions[item_idx][last_idx]
#             last_sentence_embeddings = question_model(last_sentences).pooler_output
...
#     # pdb.set_trace()
#     ## accuracy
#     print(f'Top-{flags.topk} Retrieval accuracy Middle: {num_correct_preds / len(dev_dataset)}')